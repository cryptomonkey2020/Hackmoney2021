...
function AaveAction({
  ...
  const handleModalOk = async () => {
    ...
    if(type === "deposit") {
      ...
      console.log(type, _address, assetData.underlyingAsset, assetData.symbol)
      _requests = await faucet.mint({userAddress: _address, reserve: assetData.underlyingAsset, tokenSymbol: assetData.symbol})
    }
    ...
    notification.open({
      ...
      <>
        ...
        <Text copyable>{_transactions[_transactions.length - 1] && _transactions[_transactions.length - 1].hash}</Text>
      </>,
    });
    getTokenBalance()
