...
function Lend({ selectedProvider, ethPrice }) {
  ...
  if(customProvider) {
    ...
    faucet = txBuilder.faucetService;
  }

  const [liveAsset, setLiveAsset] = useState()

  const { address, assetData, userAccountData, userConfiguration,  userAssetList, userAssetData, contracts } = useAaveData({ selectedProvider, liveAsset })
  ...
  // },
  {
    ...
    render: value => {
      ...
      {<AaveAction selectedProvider={selectedProvider} lendingPool={lendingPool} setLiveAsset={setLiveAsset} type="deposit" assetData={value} assetPrice={value.price.priceInEth} signer={signer} lendingPoolContract={lendingPoolContract} userAccountData={userAccountData} userAssetData={userAssetData[value.symbol]}/>}
      
      {<AaveAction selectedProvider={selectedProvider} lendingPool={lendingPool} faucet={faucet} setLiveAsset={setLiveAsset} type="faucet" assetData={value} assetPrice={value.price.priceInEth} signer={signer} lendingPoolContract={lendingPoolContract} userAccountData={userAccountData} userAssetData={userAssetData[value.symbol]}/>}
      </>)
    }
  },
];
  ...
  return (
    ...
    <Divider/>
        ...
        dataSource={assetData}
          // dataSource={
          //   assetData.filter(asset => (showActiveAssets&&userAssetList)
          //   ?Object.keys(userAssetList).includes(asset.symbol)
          //   :true)
          // } 
          columns={columns} 
          pagination={false} 
